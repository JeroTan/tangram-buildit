...
4. **Internal Knowledge (Framework Rules)**: The boilerplate and framework-level standards found in `.opencode/context/**`.
...
1. **Step 1: Check State & Knowledge Scan**
   ...
   - **Knowledge Scan**: Scan `tangram/knowledge/**` (if it exists) for project-specific rules and standards, and the corresponding `.opencode/context/` subdirectories for framework standards.
