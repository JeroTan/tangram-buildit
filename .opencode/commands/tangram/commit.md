...
2. **Git Knowledge Rules**: Branching rules in `.opencode/context/git/git-branching.md` and commit conventions in `.opencode/context/git/git-commit.md`.
...
2. **Branching Strategy (Enforcing git-branching.md)**
   - Review `.opencode/context/git/git-branching.md`.
...
3. **Draft the Commit (Enforcing git-commit.md)**
   ...
   - Generate a suggested commit message strictly following the Conventional Commits format found in `.opencode/context/git/git-commit.md` (e.g., `feat(auth): implement JWT validation logic`).
